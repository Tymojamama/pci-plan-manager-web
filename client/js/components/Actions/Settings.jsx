...
var Action = React.createClass({
  ...
  componentWillMount: function() {
    this.getState(this.props.id, function(state) {
      this.setState({action: state.action, pageIndex: 0});
    }.bind(this));
  },
  ...
  render: function() {
    ...
    pages[1] = (<Parameters action={this.state.action} handleChange={this.handleChange_Action}/>);

    pages[2] = (<Steps action={this.state.action} handleChange={this.handleChange_Action}/>);
  ...
  getState: function(id, callback) {
    if (id) {
      ...
      callback({action: this._action});
    }
  },

  handleChange_Action: function(action) {
    this._action = action;
    this.setState({action: this._action, pageIndex: this.state.pageIndex});
  },
  ...
  handleClick_ActionDetails: function() {
    this.setState({action: this.state.action, pageIndex: 0});
  },

  handleClick_Parameters: function() {
    this.setState({action: this.state.action, pageIndex: 1});
  },

  handleClick_Steps: function() {
    this.setState({action: this.state.action, pageIndex: 2});
  },
