...
var FeedItem = React.createClass({
  getInitialState: function() {
    return {iconSrc: '', isDeleted: ''}
  },

  componentWillMount: function() {
    if (this.props.iconSrc) {
      return this.setState({iconSrc: this.props.iconSrc, isDeleted: false});
    }

    getState(this.props.object, this.props.type, function(state) {
      this.setState({iconSrc: state.iconSrc, isDeleted: false});
    }.bind(this));
  },
      ...
      <span id={getId(this.props)}>
        <div style={Style.container}>
          <div className="container-fluid" style={Style.headingContainer}>
            <div className="col-lg-1 col-md-1 col-sm-1 col-xs-2" style={{
              padding: "0px"
            }}>{this.getIcon()}</div>
            <div className="col-lg-10 col-md-10 col-sm-10 col-xs-9" style={Style.labelContainer}>
              <div style={{
                ...
                wordWrap: "normal",
                whiteSpace: "normal"
              }} className="btn btn-link" onClick={this.handleLabelClick}>
                <b>{this.getHeading()}</b>
              </div>
              {this.getSubHeading()}
            </div>
            <div className="col-lg-1 col-md-1 col-sm-1 col-xs-1" style={{
              padding: "0px",
              textAlign: "right"
            }}>
          ...
          <div id={getComponentId(this.props.object, this.props.type)} style={{
            display: "none"
          }}>
  ...
  handleClick_Delete: function() {
    this.setState({iconSrc: this.state.iconSrc, isDeleted: true});
  },
  ...
  loadModalWindow: function() {
    ...
    if (getParameterByName('action') == 'open-task' && getParameterByName('id') == this.props.object._id) {
      ...
      return (<ModalWindow content={content} parentPath={this.props.linkPath.split("?")[0]}/>)
    }
  }
});
